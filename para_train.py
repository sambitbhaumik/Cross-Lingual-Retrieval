...
def align_qa_pairs(dataset, order=['question', 'answer']):
    def reorder_qa_pairs(example):
        new_qa_pairs = []
        for pair in example['qa_pairs']:
            new_pair = {key: pair[key] for key in order}
            new_qa_pairs.append(new_pair)
        return {'qa_pairs': new_qa_pairs}
    
    return dataset.map(reorder_qa_pairs)

def align_datasets(dataset_list, column_order=['question', 'answer']):
    aligned_datasets = [align_qa_pairs(ds, column_order) for ds in dataset_list]
...
def main():
    ...
    folder_path = "/data/users/sbhaumik/mfaq/labse15/nethome/sbhaumik"
    file_list = [load_from_disk("./labse15/nethome/sbhaumik/"+f) for f in os.listdir(folder_path) if not f.startswith('.')]
    
    eval1 = load_from_disk("./hr-trans/en_hr_trans")
    eval2 = load_from_disk("./hr-trans/fr_hr_trans")
    eval3 = load_from_disk("./hr-trans/es_hr_trans")
    eval4 = load_from_disk("./hr-trans/it_hr_trans")
    eval5 = load_from_disk("./hr-trans/nl_hr_trans")

    eval_df = [eval1, eval2, eval3, eval4, eval5]
    eval_datasets = align_datasets(eval_df)
